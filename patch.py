...
# 1. States
content = re.sub(
    r'  const \[showConfirmModal, setShowConfirmModal\] = useState\(false\);',
...
# 2. Map Modal Handler
content = re.sub(
    r'  // Handle opening confirm modal',
    r'  // Handle opening map modal\n  const openMapModal = (booking: Booking) => {\n    setSelectedBookingForMap(booking);\n    setShowMapModal(true);\n  };\n\n  // Handle opening confirm modal',
    content
)

# 3. Action Buttons
action_buttons = r"""                    <div className="flex gap-2 flex-wrap justify-end">
                      <button
...
# Fix tel link
content = re.sub(
    r'href={`tel:\$\{booking\.customer_email\}`}',
    r'href={`tel:${booking.customer_phone || \'\'}`}',
    content
)
