...
export default function FloatingContactButtons() {
  ...
  return (
    <>
      ...
      {/* Mobile Version - WhatsApp FAB only (Call is in bottom nav) */}
      {/* Positioned above the bottom nav bar (bottom nav is ~68px + env safe area) */}
      <div className="md:hidden fixed z-40" style={{ bottom: 'calc(68px + env(safe-area-inset-bottom, 0px) + 12px)', right: '16px' }}>
        {/* Expanded Buttons */}
        {isExpanded && (
          <div className="absolute bottom-16 right-0 flex flex-col gap-3 animate-fade-in-up">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-end gap-2 active:scale-95 transition-transform"
              onClick={() => setIsExpanded(false)}
            >
              <span className="bg-white text-slate-700 text-xs font-bold px-3 py-1.5 rounded-full shadow-md whitespace-nowrap">
                WhatsApp
              </span>
              <div className="w-12 h-12 rounded-full bg-green-500 flex items-center justify-center shadow-xl shadow-green-500/40">
        ...
        <button
          ...
          className={`flex items-center justify-center w-13 h-13 rounded-full shadow-2xl transition-all duration-300 ${
            isExpanded
              ? 'bg-red-500 rotate-45 scale-95'
              : 'bg-green-500 hover:scale-110'
          }`}
          style={{ width: 52, height: 52 }}
          aria-label={isExpanded ? "Close contact menu" : "Open WhatsApp"}
        >
          {isExpanded ? (
